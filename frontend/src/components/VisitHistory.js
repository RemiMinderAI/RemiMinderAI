...
const VisitHistory = () => {
  ...
  const [userId, setUserId] = useState("9957ecad-f983-4211-a1e1-41a67b0a0621");
...
useEffect(() => {
    const fetchUser = async () => {
      ...
      if (error || !user) {
        console.warn("No user session found:", error?.message);
        navigate("/sign-in");
        return;
      }
...
useEffect(() => {
  ...
  const fetchVisits = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/visit-summaries?user_id=${userId}`);
      ...
      console.error("Failed to fetch visit summaries:", e);
      setError("Failed to load visit history. Is the backend running?");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className={styles.content}>
        ...
        <div className={styles.statCard}>
            ...
            <div>
                ...
                <div className={styles.statValue}>39 min</div>
            </div>
