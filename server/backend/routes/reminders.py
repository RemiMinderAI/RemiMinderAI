...
async def get_reminder(reminder_id: str, patient_id: str):
    """
    Get a single reminder by ID.
    """
    try:
        reminder = await get_reminder_by_id(reminder_id, patient_id)
...
async def update_reminder(reminder_id: str, patient_id: str, updates: ReminderUpdate):
    """
    Update a reminder (time, title, status, etc.).
    """
    try:
        reminder = await update_reminder_details(reminder_id, patient_id, updates)
...
async def delete_reminder(reminder_id: str, patient_id: str):
    """
    Cancel/delete a reminder.
    """
    try:
        success = await cancel_reminder(reminder_id, patient_id)
...
async def mark_complete(reminder_id: str, patient_id: str, action: ReminderAction = ReminderAction()):
    """
    Mark a reminder as completed.
    """
    try:
        reminder = await complete_reminder(reminder_id, patient_id, action)
...
async def snooze_reminder_post(
    ...
    """
    try:
        reminder = await snooze_reminder(reminder_id, patient_id, snooze_minutes)
...
async def skip_reminder_post(reminder_id: str, patient_id: str, action: ReminderAction = ReminderAction()):
    """
    Skip a reminder with optional reason.
    """
    try:
        reminder = await skip_reminder(reminder_id, patient_id, action)
...
async def get_caregiver_activity(caregiver_id: str, patient_id: str):
    """
    Get aggregated activity data for caregiver dashboard.
    Shows next reminders, recent activity, and alert summary.
    """
    try:
        dashboard_data = await get_caregiver_dashboard_data(caregiver_id, patient_id)
